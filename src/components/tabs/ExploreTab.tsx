  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 pb-20 relative overflow-hidden">
      <div className="absolute top-4 inset-x-4 z-[1000] space-y-3 pointer-events-none">
        ...
        <div className="flex justify-center pointer-events-auto">
          <div className="bg-white/90 backdrop-blur-md p-1.5 rounded-2xl shadow-xl border border-white flex gap-1">
            <button onClick={() => setViewMode("map")} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold transition-all ${viewMode === "map" ? "bg-[#009688] text-white" : "text-slate-500"}`}>
              <MapIcon className="w-4 h-4" /> Mapa
            </button>
            <button onClick={() => setViewMode("list")} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold transition-all ${viewMode === "list" ? "bg-[#009688] text-white" : "text-slate-500"}`}>
            ...
            {filteredPlaces.map(place => (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={place.id} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100">
                <div className="relative h-32">
                   ...
                   <div className="absolute top-2 right-2 bg-white/90 backdrop-blur px-2 py-1 rounded-lg text-[9px] font-black text-[#009688] uppercase">
                     {place.type}
                   </div>
                </div>
                <div className="p-4 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{place.tag} • {place.barrio}</p>
                  </div>
                  <div className="flex items-center text-yellow-500 font-bold text-sm">
                    <Star className="w-3.5 h-3.5 fill-current mr-0.5" />{place.rating}
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      {viewMode === "map" && (
        <button 
          onClick={handleGPS} 
          className="absolute bottom-10 right-6 p-4 bg-[#009688] rounded-full shadow-2xl text-white z-[1001] active:scale-90 transition-all shadow-[#009688]/40 pointer-events-auto"
        >
          ...
          <>
            ...
            >
              ...
              <div className="space-y-8">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Seguridad</p>
                  <div className="flex flex-wrap gap-2">
